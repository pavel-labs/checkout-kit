...
## Два хвоста в словаре

Оба объявлены в домене и не поддержаны ничем — и оба стоит знать до того, как писать под них
плагин:
...
**`ActionSurface: 'popup'` мёртв.** Он есть в юнионе и в скриптовых раннерах
`@checkout-kit/testing` — и больше нигде: ни один браузерный раннер его не поддерживает и ни
один плагин его не возвращает.
