...
## Как получить доступ к пакетам

Они публикуются **приватно**, в GitHub Packages, поэтому просто `npm install` их не найдёт.
Проекту надо сказать, где живёт scope, и дать токен на чтение.
...
Хватит классического personal access token со скоупом **`read:packages`**. Держите его в
переменных окружения, а не в файле: `.npmrc` коммитится, токены — нет.

Всё, что вне scope `@checkout-kit`, по-прежнему тянется из публичного реестра.
